# scraper.py - DOKA Exclave VPN | Ultimate Ultra Glass
import requests
import re
import random
import json
...
def run_doka_exclave():
    ...
    try:
        ...
        print(f"🔄 [{current_time.strftime('%H:%M:%S')}] كشط Exclave VPN...")
        ...
        patterns = {
            'vmess':      r'exclave://vmess[^\s<"\'<>]+',
            'vless':      r'exclave://vless[^\s<"\'<>]+',
            'trojan':     r'exclave://trojan[^\s<"\'<>]+',
            'ss':         r'exclave://ss[^\s<"\'<>]+',
            'ssh':        r'exclave://ssh[^\s<"\'<>]+',
            'hysteria2':  r'exclave://hysteria2[^\s<"\'<>]+',
        }
        ...
        servers_by_protocol = {"vmess": [], "vless": [], "trojan": [], "ss": [], "ssh": [], "hysteria2": []}
        countries_count = {}
        all_servers_data = []
        deleted_count = 0
        active_count = 0
        ...
        for link in clean_links:
            ...
            if "exclave://vmess" in link_lower:
                ...
                proto_type = "SS"
            elif "exclave://ssh" in link_lower:
                proto_type = "SSH"
            elif "exclave://hysteria2" in link_lower:
            ...
            ping_ranges = {
                ...
                'SS': (70, 220), 'SSH': (30, 120), 'HYSTERIA2': (20, 100)
            }
        ...
        print(f"\n{'='*50}")
        print(f"✅ [DOKA EXCLAVE] تم!")
        print(f"   📊 {total_servers} سيرفر | 🆕 {new_count} | 🗑️ {deleted_count}")
...
<head>
    ...
    <title>DOKA Exclave VPN | حرية التصفح</title>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .glass {{ background: rgba(255,255,255,0.03); backdrop-filter: blur(40px) saturate(180%); -webkit-backdrop-filter: blur(40px) saturate(180%); border: 1px solid rgba(255,255,255,0.08); }}
        .glass-card {{
            background: rgba(255,255,255,0.02); backdrop-filter: blur(30px) saturate(180%); -webkit-backdrop-filter: blur(30px) saturate(180%);
        ...
        .glass-card.new-server {{ animation: newPulse 2.5s ease-in-out infinite; }}
        @keyframes newPulse {{ 0%,100% {{ box-shadow: 0 0 0 0 rgba(34,197,94,0.5); }} 50% {{ box-shadow: 0 0 40px 10px rgba(34,197,94,0.15); }} }}
        .glass-nav {{ background: rgba(6,6,15,0.5); backdrop-filter: blur(40px); border-bottom: 1px solid rgba(255,255,255,0.05); }}
        .tab-btn {{ background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); color: #94a3b8; transition: all 0.4s; backdrop-filter: blur(10px); }}
        ...
        .btn-primary {{ background: linear-gradient(135deg, rgba(99,102,241,0.6), rgba(139,92,246,0.6)); border: 1px solid rgba(255,255,255,0.1); color: white; transition: all 0.4s; box-shadow: 0 4px 20px rgba(99,102,241,0.3); }}
        ...
        .btn-glass {{ background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); color: #e2e8f0; transition: all 0.4s; backdrop-filter: blur(10px); }}
        .btn-glass:hover {{ background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.2); }}
        .badge-new {{ background: linear-gradient(135deg, #22c55e, #10b981); color: white; font-size: 0.6rem; padding: 3px 10px; border-radius: 30px; animation: badgeGlow 2s ease-in-out infinite; }}
        ...
        .toast {{ background: rgba(15,15,30,0.8); backdrop-filter: blur(30px); border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 20px 50px rgba(0,0,0,0.5); }}
        .link-preview {{ background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.06); border-radius: 18px; font-family: monospace; }}
        .confetti {{ position: fixed; pointer-events: none; z-index: 9999; animation: confettiFall 1.2s cubic-bezier(0.25,0.46,0.45,0.94) forwards; }}
...
<body class="antialiased relative z-10">
    ...
    <nav class="glass-nav sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-5 py-4 flex flex-wrap justify-between items-center text-sm gap-4">
            <div class="flex items-center gap-4">
                ...
                <span class="hidden sm:inline text-gray-400 text-xs">| EXCLAVE VPN</span>
            </div>
    ...
    <section class="relative py-16 md:py-24 text-center px-4">
        <div class="max-w-4xl mx-auto">
            ...
            <p class="text-gray-400 text-lg md:text-xl mb-3">سيرفرات حصرية · آمنة · محدثة تلقائياً</p>
            ...
            <div class="flex flex-wrap justify-center gap-4 mb-6">
                ...
                <div class="stat-card px-6 py-4 text-center min-w-[100px]">
                    <span class="text-3xl font-black text-green-400" style="text-shadow: 0 0 30px rgba(34,197,94,0.5);">{active_count}</span>
    ...
    <section class="max-w-7xl mx-auto px-4 py-8">
        <h2 class="text-xl font-bold mb-6 text-gray-300 flex items-center gap-3">
            ...
            سيرفرات Exclave VPN
            <span class="text-xs text-gray-600">· 🟢{active_count} 💤{idle_count} ⚠️{warning_count}</span>
            <span class="text-xs text-gray-600" id="last-copied-info"></span>
        </h2>
        ...
        <div id="no-servers-msg" class="text-center py-20 text-gray-600 hidden">
            ...
            <p class="text-sm">لا توجد سيرفرات</p>
        </div>
    </section>

    <footer class="border-t border-white/5 mt-16">
        <div class="max-w-7xl mx-auto px-4 py-10 text-center">
            <p class="text-gray-600 text-xs">© 2026 DOKA Exclave VPN</p>
            <p class="text-gray-700 text-[10px] mt-1.5">تحديث كل 3 ساعات · ⏳ حذف بعد {max_age_hours} ساعة</p>
            <button id="show-stats-btn" class="mt-5 glass px-6 py-2.5 rounded-full text-xs text-gray-400 hover:text-white transition-all">
    ...
    <div id="qr-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70 backdrop-blur-sm" onclick="closeQRModal(event)">
        <div class="glass-card p-8" onclick="event.stopPropagation()" style="opacity:1;transform:none;">
            <div id="qr-modal-content" class="flex justify-center bg-white p-4 rounded-xl"></div>
    ...
    <script>
        ...
        function spawnConfetti() {{
            const colors = ['#6366f1','#ec4899','#22c55e','#f59e0b','#3b82f6','#a855f7','#fbbf24'];
            for (let i=0; i<40; i++) {{
                const c = document.createElement('div'); c.className = 'confetti';
                c.style.left = Math.random()*100+'%'; c.style.top = '-20px';
                c.style.width = Math.random()*10+4+'px'; c.style.height = Math.random()*10+4+'px';
        ...
        window.copyText = (text) => {{
            navigator.clipboard.writeText(text).then(() => {{
                showToast('✅ تم النسخ! 🎉'); spawnConfetti();
        ...
        window.showQR = (link) => {{
            ...
            c.innerHTML = ''; new QRCode(c, {{ text: link, width: 220, height: 220, colorDark: "#1e293b", colorLight: "#ffffff" }});
        ...
        function renderServers(filter) {{
            ...
            filtered.forEach((srv, i) => {{
                ...
                let statusBadge = srv.status === 'active' ? '<span class="badge-active text-[0.6rem] px-2.5 py-1 rounded-full font-bold">🟢 نشط</span>' :
                                  srv.status === 'idle' ? '<span class="badge-idle text-[0.6rem] px-2.5 py-1 rounded-full font-bold">💤 خامل</span>' :
                                  '<span class="badge-warning text-[0.6rem] px-2.5 py-1 rounded-full font-bold">⚠️ خطر</span>';
                ...
                h += `<div class="glass-card p-5 ${{srv.is_new ? 'new-server' : ''}}" style="animation-delay:${{i*0.07}}s;">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="bg-gradient-to-r ${{srv.proto_gradient}} text-white text-[0.65rem] font-bold px-3 py-1 rounded-full uppercase">${{srv.proto}}</span>
                    ...
                    <div class="flex gap-2">
                        <button onclick="copyText('${{srv.link}}')" class="flex-1 btn-primary py-2.5 rounded-xl text-xs font-semibold"><i class="far fa-copy ml-1.5"></i> نسخ</button>
        ...
        (function() {{
            ...
            for (let i=0; i<20; i++) {{
                ...
                orb.style.cssText = `width:${{Math.random()*60+20}}px;height:${{Math.random()*60+20}}px;left:${{Math.random()*100}}%;background:${{cols[Math.floor(Math.random()*cols.length)]}};animation-duration:${{Math.random()*25+20}}s;animation-delay:${{Math.random()*20}}s;filter:blur(${{Math.random()*8+4}}px);`;
                co.appendChild(orb);
            }}
        }})();
        ...
        document.getElementById('show-stats-btn').addEventListener('click', () => {{
            document.querySelector('nav').style.display='none'; document.querySelector('section').style.display='none';
            document.getElementById('filter-tabs').style.display='none'; document.getElementById('servers-grid').parentElement.style.display='none';
            document.querySelector('footer').style.display='none'; document.querySelector('.aurora-container').style.display='none';
            document.querySelector('.orbs-container').style.display='none'; document.getElementById('stats-page').classList.remove('hidden');
            ...
            chartInstances.proto = new Chart(ctx1, {{
                type: 'doughnut', data: {{ labels: Object.keys(statsData.by_protocol).map(p=>p.toUpperCase()), datasets: [{{ data: Object.values(statsData.by_protocol), backgroundColor: ['#f97316','#3b82f6','#a855f7','#22c55e','#64748b','#f43f5e'], borderColor: 'rgba(255,255,255,0.05)', borderWidth: 4 }}] }},
                options: {{ responsive: true, cutout: '65%', plugins: {{ legend: {{ position:'bottom', labels: {{ color:'#94a3b8', padding:16, font:{{family:'Tajawal',size:11}} }} }} }} }}
            }});
            ...
            chartInstances.country = new Chart(ctx2, {{
                type: 'polarArea', data: {{ labels: Object.keys(co2), datasets: [{{ data: Object.values(co2), backgroundColor: ['#6366f1','#ec4899','#22c55e','#f59e0b','#3b82f6','#ef4444','#a855f7','#06b6d4'], borderColor: 'rgba(255,255,255,0.05)', borderWidth: 3 }}] }},
                options: {{ responsive: true, scales: {{ r: {{ ticks: {{ display:false }}, grid: {{ color:'rgba(255,255,255,0.03)' }} }} }}, plugins: {{ legend: {{ position:'bottom', labels: {{ color:'#94a3b8', padding:14, font:{{family:'Tajawal',size:10}} }} }} }} }}
            }});
        }});
        ...
        document.getElementById('clear-fav-btn').addEventListener('click', () => {{
            if (confirm('حذف كل المفضلة؟')) {{ localStorage.removeItem('doka_exclave_fav'); updateFavCount(); if (currentFilter==='favorites') renderServers('all'); else renderServers(currentFilter); showToast('تم الحذف 🗑️'); }}
        }});
